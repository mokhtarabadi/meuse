...
(defn get-crate-index
  ...
  [crate-db request crate-name]
  (try
    (let [versions (public-crate/get-crate-and-versions crate-db crate-name)]
      (if (seq versions)
...
(defn get-index-one-char
  ...
  (let [uri (:uri request)
    ...
    (if (and match (>= (count match) 2))
      (get-crate-index crate-db request (nth match 1))
      (ring/not-found {:status "not_found"
...
(defn get-index-three-char
  ...
  (let [uri (:uri request)
    ...
    (if (and match (>= (count match) 3))
      (get-crate-index crate-db request (nth match 2))
      (ring/not-found {:status "not_found"
...
(defn get-index-crate
  ...
  (let [uri (:uri request)
    ...
    (if (and match (>= (count match) 4))
      (get-crate-index crate-db request (nth match 3))
      (ring/not-found {:status "not_found"
